...
class 専攻科_授業料以外_奨学給付金(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        低所得世帯限度額 = parameters(対象期間).福祉.育児.高等学校奨学給付金.所得割額_低所得世帯
        多子世帯限度額 = parameters(対象期間).福祉.育児.高等学校奨学給付金.所得割額_多子世帯
        多子世帯扶養人数 = parameters(対象期間).福祉.育児.高等学校奨学給付金.多子世帯_扶養人数

        低所得世帯である = 所得割額合計 <= 低所得世帯限度額
        多子世帯である = (所得割額合計 <= 多子世帯限度額) * (扶養人数 >= 多子世帯扶養人数)

        支給対象 = 子供である * 高校生である * 専攻科である * np.logical_not(住民税非課税世帯である) * (低所得世帯である + 多子世帯である)
